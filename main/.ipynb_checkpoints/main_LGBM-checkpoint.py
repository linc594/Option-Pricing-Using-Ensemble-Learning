...
import pandas as pd
import numpy as np
from model.LGBM import train_model, predict, evaluate_model, save_model
        ...
        for ne in param_grid['n_estimators']:
            for nl in param_grid['num_leaves']:
                for md in param_grid['max_depth']:
                    model = lgb.LGBMRegressor(learning_rate=lr, n_estimators=ne, num_leaves=nl, max_depth=md)
                    ...
                    if mse < best_mse:
                        best_mse = mse
                        best_params = {'learning_rate': lr, 'n_estimators': ne, 'num_leaves': nl, 'max_depth': md}
...
if use_hyperparameter_tuning:
    ...
    best_params = {'learning_rate': 0.1, 'n_estimators': 100, 'num_leaves': 31, 'max_depth': -1}
...
save_model(model, '../model/save/lgbm_model.pkl')
