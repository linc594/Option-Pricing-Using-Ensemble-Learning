    ...
    for lr in param_grid['learning_rate']:
        for it in param_grid['iterations']:
            for dp in param_grid['depth']:
                model = CatBoostRegressor(learning_rate=lr, iterations=it, depth=dp, verbose=0)
                ...
                if mse < best_mse:
                    best_mse = mse
                    best_params = {'learning_rate': lr, 'iterations': it, 'depth': dp}
...
if use_hyperparameter_tuning:
    ...
    best_params = {'iterations': 1200, 'learning_rate': 0.05, 'depth': 9}
...
# 日志记录
log_experiment(log_path, "CatBoost", best_params, metrics, save_path)